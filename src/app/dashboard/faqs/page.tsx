...
export default function FAQPage() {
  ...
  const contextTags = [
    ...
    { id: "profile-editing", name: "Profile" },
    { id: "mentor-search", name: "Mentor Search" },
  ...
  useEffect(() => {
    const getAiResponse = async () => {
      ...
      if (searchQuery && filteredFAQs.length === 0) {
        setIsLoadingAi(true);
        try {
          const response = await axios.post("https://medical-backend-3eek.onrender.com/api/test/answer", {
    ...
    <div className="min-h-screen ">
      <main className="container py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <AutoSuggestionEngine searchQuery={searchQuery} />
          ...
          {/* Contextual FAQ Demo Section */}
          <div className="border rounded-lg p-6 mb-12">
            <h2 className="text-2xl font-bold mb-6">Contextual FAQ </h2>
            <p className="text-muted-foreground mb-6">
              This demonstrates how the ContextualFAQ component can be used to show relevant FAQs based on the user&apos;s
              current context.
            </p>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {contextTags.map((tag) => (
                <Button
                  key={tag.id}
                  variant={selectedContextDemo === tag.id ? "default" : "outline"}
                  onClick={() => setSelectedContextDemo(tag.id)}
                >
                  {tag.name}
                </Button>
              ))}
            ...
            <div className="border rounded-lg p-4 bg-muted/30">
              <p className="text-sm text-muted-foreground mb-4">
                <strong>Example:</strong> This FAQ would appear in the{" "}
                {contextTags.find((t) => t.id === selectedContextDemo)?.name} section of the application.
              </p>
              <ContextualFAQ contextTag={selectedContextDemo} />
            </div>
          </div>
          ...
          <div className="bg-muted rounded-lg p-8 text-center mt-16">
            ...
            <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
              If you couldn&apos;t find the answer to your question, our support team is here to help. Feel free to
              reach out to us.
            </p>
