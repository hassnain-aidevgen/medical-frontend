...
const apiClient = axios.create({
  baseURL: "https://medical-backend-3eek.onrender.com/api/v2",
  headers: {
...
apiClient.interceptors.request.use(
  (config) => {
    try {
      ...
      if (!userId) {
        console.warn(
          "No user ID found in localStorage. Some API calls may fail."
        );
        return config;
      }
...
export const apiService = {
  ...
  async getFlashcardById(id: string): Promise<ApiResponse<Flashcard>> {
    try {
      ...
      const response = await apiClient.get<RawFlashcardData>(
        `/flashcards/${id}`
      );
  ...
  async createFlashcard(
    ...
    try {
      ...
      const response = await apiClient.post<RawFlashcardData>(
        "/flashcards",
        dataWithDefaults
      );
  ...
  async updateFlashcard(
    ...
    try {
      ...
      const response = await apiClient.put<RawFlashcardData>(
        `/flashcards/${id}`,
        flashcardData
      );
...
function normalizeFlashcard(flashcard: RawFlashcardData): Flashcard {
  ...
  return {
    ...
    difficulty: (flashcard.difficulty || "medium") as
      | "easy"
      | "medium"
      | "hard",
    ...
    reviewCount:
      typeof flashcard.reviewCount === "number" ? flashcard.reviewCount : 0,
    lastReviewedDate: flashcard.lastReviewedDate
      ? new Date(flashcard.lastReviewedDate)
      : null,
    nextReviewDate: flashcard.nextReviewDate
      ? new Date(flashcard.nextReviewDate)
      : null,
    reviewStage:
      typeof flashcard.reviewStage === "number" ? flashcard.reviewStage : 0,
    reviewPriority:
      (flashcard.reviewPriority as "low" | "medium" | "high" | null) || null,
