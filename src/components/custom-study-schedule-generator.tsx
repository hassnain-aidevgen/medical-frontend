...
const CustomStudyScheduleGenerator: React.FC<StudyScheduleProps> = ({ selectedExam, examDate, userId }) => {
  ...
  useEffect(() => {
    ...
    const proficiencyResults: SubjectProficiency[] = Object.entries(subjectGroups).map(([subject, subjectTests]) => {
      ...
      const proficiencyScore = totalTests > 0 ? Math.max(10, Math.round((completedTests / totalTests) * 100)) : 10 // Minimum 10% proficiency
    ...
    examBlueprint.forEach((item) => {
      const existingSubject = proficiencyResults.find((p) => p.subject.toLowerCase() === item.topic.toLowerCase())
  ...
  const generateStudySchedule = () => {
    ...
    try {
      ...
      const subjectsWithPriority = examBlueprint.map((blueprint) => {
        ...
        const proficiency = proficiencyData.find((p) => p.subject.toLowerCase() === blueprint.topic.toLowerCase()) || {
  ...
  const getSubjectColor = (subject: string): string => {
    ...
    const existingTest = tests.find((test) => test.subjectName.toLowerCase() === subject.toLowerCase())
            ...
            <div>
              ...
              <div className="mb-4 p-3 bg-blue-50 rounded-md">
                <p className="text-sm text-blue-800">
                  This schedule prioritizes subjects with high exam weight and lower proficiency. Study sessions are
                  distributed across {daysUntilExam} days until your exam.
                </p>
              </div>
              ...
              <div className="overflow-hidden rounded-lg border border-gray-200">
                ...
                {studySchedule.slice(0, showFullSchedule ? undefined : 7).map((day, index) => (
                  <div
                    key={index}
                    className={`${index % 2 === 0 ? "bg-white" : "bg-gray-50"} p-3 border-b last:border-b-0`}
                  >
