...
const SubjectPrioritization: React.FC<SubjectPrioritizationProps> = ({ selectedExam, userId, examDate }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          ...
          console.log("Blueprint API response status:", blueprintResponse.status)
          ...
          if (blueprintResponse.data && Array.isArray(blueprintResponse.data)) {
            ...
            console.error("Invalid blueprint data format:", blueprintResponse.data)
          ...
          if (axios.isAxiosError(blueprintError)) {
            console.error("API Error Response:", blueprintError.response?.data)
            console.error("API Error Status:", blueprintError.response?.status)
            console.error("API Error URL:", blueprintError.config?.url)
          }

          toast.error("Failed to fetch exam blueprint data. Using fallback data.")

          // Fallback blueprint data
          blueprintData = getFallbackBlueprintData(selectedExam)
          console.log("Using fallback blueprint data:", blueprintData)
        }
  ...
  const calculateSubjectPriorities = useCallback(() => {
    try {
      ...
      const priorityData: SubjectPriority[] = examBlueprint.map((blueprint) => {
        ...
        const proficiencyScore = testsTotal > 0 ? Math.max(10, Math.round((testsCompleted / testsTotal) * 100)) : 10
        ...
        if (daysToExam <= 14) {
          suggestedDifficulty = proficiencyScore >= 70 ? "advanced" : proficiencyScore >= 40 ? "moderate" : "foundation"
        } else if (daysToExam <= 30) {
          suggestedDifficulty = proficiencyScore >= 80 ? "advanced" : proficiencyScore >= 50 ? "moderate" : "foundation"
        } else {
          suggestedDifficulty = proficiencyScore >= 90 ? "advanced" : proficiencyScore >= 60 ? "moderate" : "foundation"
        }
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                  ...
                  <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-lg p-4 border border-red-200">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="text-xl font-bold text-gray-900">{subjectPriorities[0].subject}</div>
                      </div>
                    ...
                    <div className="mb-2 text-sm">
                      ...
                      <span
                        className={`${subjectPriorities[0].proficiencyScore < 50 ? "text-red-600" : "text-green-600"}`}
                      >
                    ...
                    <div className="bg-gradient-to-br from-orange-50 to-yellow-50 rounded-lg p-4 border border-orange-200">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="text-xl font-bold text-gray-900">{subjectPriorities[1].subject}</div>
                        </div>
                      ...
                      <div className="mb-2 text-sm">
                        <span className="font-medium">Proficiency: </span>
                        <span
                          className={`${subjectPriorities[1].proficiencyScore < 50 ? "text-red-600" : "text-green-600"}`}
                        >
                          {subjectPriorities[1].proficiencyScore}%
                        </span>
                      </div>
                      <div className="text-xs text-orange-600 mt-1">Focus after top priority</div>
                    </div>
                  )}
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                    ...
                    <div className="text-xl font-bold text-gray-900">
                      ...
                      {subjectPriorities.filter((s) => s.isHighYield).length}
                    </div>
                    ...
                    <div className="text-xs text-blue-600 mt-1">Focus on high-yield subjects first</div>
                  </div>
                  ...
                  {examDate && (
                    <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-4 border border-purple-200">
                      ...
                      <div className="mt-2 text-sm">{formatDate(examDate)}</div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {subjectPriorities.slice(0, showAll ? undefined : 5).map((subject, index) => (
                    <div
                      key={subject.subject}
                    ...
                    >
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-3">
                        ...
                        <div className="flex space-x-3 mt-2 md:mt-0">
                          <span
                            className={`px-2 py-1 rounded-full text-xs ${getDifficultyBgColor(subject.suggestedDifficulty)}`}
                          >
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div
                              className={`h-2 rounded-full ${
                                subject.proficiencyScore >= 70
                                  ? "bg-green-500"
                                  : subject.proficiencyScore >= 40
                                    ? "bg-yellow-500"
                                    : "bg-red-500"
                              }`}
                        ...
                        <div>
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div
                              ...
                              style={{
                                width: `${Math.min(100, (subject.priorityScore / subjectPriorities[0].priorityScore) * 100)}%`,
                              }}
                            ></div>
                          </div>
              ...
              <div className="mt-6 bg-blue-50 p-4 rounded-lg">
                ...
                <ul className="space-y-2">
                      ...
                      <span>
                        {subjectPriorities.filter((s) => s.isHighYield && s.proficiencyScore < 70).length > 0
                          ? `Focus on the high-yield subjects that need improvement: ${subjectPriorities
                              ...
                              .join(
                                " and ",
                              )}${subjectPriorities.filter((s) => s.isHighYield && s.proficiencyScore < 70).length > 2 ? " and others." : "."}`
                  ...
                  <li className="text-sm flex items-start">
                    ...
                    <span>
                      For optimal results, spend{" "}
                      {subjectPriorities.length > 0
                        ? Math.max(1, Math.min(3, Math.ceil(subjectPriorities[0].blueprintPercentage / 10)))
                        : 1}
                      -
                      {subjectPriorities.length > 0
                        ? Math.max(2, Math.min(5, Math.ceil(subjectPriorities[0].blueprintPercentage / 5)))
                        : 2}{" "}
                      hours on{" "}
                      {subjectPriorities.length > 0 ? subjectPriorities[0].subject : "your top priority subject"} before
                      moving to lower priority subjects.
                    </span>
                  </li>
                </ul>
              </div>
